...
class Settings(BaseSettings):
    ...
    # Client credentials
    client_id: str = Field(default="", description="OAuth2 client ID", validation_alias=f"{ENV_PREFIX}CLIENT_ID")
    client_secret: str = Field(
        default="", description="OAuth2 client secret", validation_alias=f"{ENV_PREFIX}CLIENT_SECRET"
    )
    redirect_uri: Optional[str] = Field(
        default=None, description="OAuth2 redirect URI", validation_alias=f"{ENV_PREFIX}REDIRECT_URI"
    )
    access_token: Optional[str] = Field(
        default=None, description="OAuth2 access token", validation_alias=f"{ENV_PREFIX}ACCESS_TOKEN"
    )
    ...
    custom_base_url: Optional[str] = Field(
        default=None, description="Custom API base URL", validation_alias=f"{ENV_PREFIX}BASE_URL"
    )
